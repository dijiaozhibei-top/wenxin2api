...
test('sieve emits tool_calls when fullwidth DSML prefix variant spans multiple chunks', () => {
  const events = runSieve(
    [
      ...
      '_calls>\n',
      '<|DSML|invoke name="Bash">\n',
      '<|DSML|parameter name="command"><![CDATA[ls -la /Users/aq/Desktop/myproject/wenxin2api/]]></|DSML|parameter>\n',
      '<|DSML|parameter name="description"><![CDATA[List project root contents]]></|DSML|parameter>\n',
      '</|DSML|invoke>\n',
      '<|DSML|invoke name="Bash">\n',
      '<|DSML|parameter name="command"><![CDATA[cat /Users/aq/Desktop/myproject/wenxin2api/package.json 2>/dev/null || echo "No package.json found"]]></|DSML|parameter>\n',
...
test('sieve keeps CDATA tool examples buffered until the outer closing tag arrives', () => {
  ...
  const chunks = [
    ...
    ']]></parameter>\n    <parameter name="file_path">wenxin2api-4.0-Release-Notes.md</parameter>\n  </invoke>\n</tool_calls>',
  ];
  const events = [];
...
test('parseToolCalls keeps XML-looking CDATA content intact', () => {
  ...
  const payload = `<tool_calls><invoke name="Write"><parameter name="content"><![CDATA[${content}]]></parameter><parameter name="file_path">wenxin2api-4.0-Release-Notes.md</parameter></invoke></tool_calls>`;
  const calls = parseToolCalls(payload, ['Write']);
  assert.equal(calls.length, 1);
  assert.equal(calls[0].input.content, content);
  assert.equal(calls[0].input.file_path, 'wenxin2api-4.0-Release-Notes.md');
});
