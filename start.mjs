...
// 编译产物路径
const BINARY = join(__dirname, isWindows ? 'wenxin2api.exe' : 'wenxin2api');
...
const CONFIG = {
  ...
  adminKey: process.env.wenxin2api_ADMIN_KEY || 'admin',
  webuiDir: join(__dirname, 'webui'),
  staticAdminDir: process.env.wenxin2api_STATIC_ADMIN_DIR || join(__dirname, 'static', 'admin'),
};
...
async function startBackendDev() {
  ...
  const proc = spawn('go', ['run', './cmd/ds2api'], {
    ...
    env: { ...process.env,
      ...
      wenxin2api_ADMIN_KEY: CONFIG.adminKey,
...
async function showMenu() {
  ...
  console.log(`  wenxin2api_ADMIN_KEY:  ${colors.cyan}${CONFIG.adminKey}${colors.reset}`);
  ...
  console.log(`${colors.dim}  自定义: wenxin2api_ADMIN_KEY=密钥 PORT=5001 node start.mjs${colors.reset}`);
...
async function main() {
  ...
  switch (cmd) {
    ...
    case '--help':
      console.log(`
${colors.bright}wenxin2api 启动脚本 (Go)${colors.reset}
...
${colors.cyan}常用环境变量:${colors.reset}
  ...
  wenxin2api_ADMIN_KEY   管理员密钥 (默认: admin)
  wenxin2api_CONFIG_PATH 配置文件路径 (默认: config.json)
  GOPROXY            Go 模块代理 (默认: https://goproxy.cn,direct)
  NPM_REGISTRY       npm 镜像源 (默认: https://registry.npmmirror.com)

${colors.cyan}示例:${colors.reset}
  wenxin2api_ADMIN_KEY=mykey PORT=8080 node start.mjs dev
