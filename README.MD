...
flowchart LR
    ...
    subgraph wenxin2api["wenxin2api 4.x（模块化 HTTP surface + PromptCompat 内核）"]
...
- **后端**：Go（`cmd/wenxin2api/`、`api/`、`internal/`），不依赖 Python 运行时
...
OpenAI `/v1/*` 仍是推荐的规范路径；同时支持 `/models`、`/chat/completions`、`/responses`、`/embeddings`、`/files`、`/files/{file_id}` 等根路径快捷路由，方便只配置 wenxin2api 根地址的第三方客户端。
...
- `ANTHROPIC_BASE_URL` 推荐直接指向 wenxin2api 根地址（例如 `http://127.0.0.1:5001`），Claude Code 会请求 `/v1/messages?beta=true`。
- `ANTHROPIC_API_KEY` 需要与 `config.json` 中 `keys` 一致；建议同时保留常规 key 与 `sk-ant-*` 形态 key，兼容不同客户端校验习惯。
- 若系统设置了代理，建议对 wenxin2api 地址配置 `NO_PROXY=127.0.0.1,localhost,<你的主机IP>`，避免本地回环请求被代理拦截。
...
- Docker / Vercel：由 `config.json` 生成 `wenxin2api_CONFIG_JSON`（Base64）注入环境变量，也可以直接写原始 JSON
...
# 2. 编辑 .env（至少设置 wenxin2api_ADMIN_KEY；如需修改宿主机端口，可额外设置 wenxin2api_HOST_PORT）
...
默认 `docker-compose.yml` 会把宿主机 `6011` 映射到容器内的 `5001`。如果你希望直接对外暴露 `5001`，请设置 `wenxin2api_HOST_PORT=5001`（或者手动调整 `ports` 配置）。
同时默认把 `./config.json` 挂载到容器 `/data/config.json`，并设置 `wenxin2api_CONFIG_PATH=/data/config.json`，用于避免 `/app` 只读导致运行时 token 持久化失败。
镜像会预创建 `/data` 并授权给非 root 的 `wenxin2api` 用户；如果使用单文件 bind mount，请确保宿主机 `config.json` 对容器用户可读写，例如 `chmod 644 config.json`。
...
2. 部署完成后访问 `/admin`，使用 Zeabur 环境变量/模板指引中的 `wenxin2api_ADMIN_KEY` 登录。
3. 在管理台导入/编辑配置（会写入并持久化到 `/data/config.json`）。

Zeabur 首次空卷启动时可以没有 `/data/config.json`；wenxin2api 会先使用空的文件模式配置启动，并在管理台首次保存时创建该文件。

不依赖模板手动部署时，在 Zeabur 中选择 GitHub 仓库服务，Root Directory 保持 `/`，使用仓库根目录 `Dockerfile` 构建；添加持久卷 `/data`，设置 `PORT=5001`、`wenxin2api_ADMIN_KEY=你的强密钥`、`wenxin2api_CONFIG_PATH=/data/config.json`，然后暴露 HTTP 端口 `5001`。更完整步骤见 [docs/DEPLOY.md](docs/DEPLOY.md#不使用模板手动部署)。
...
2. 在 Vercel 上导入项目
3. 配置环境变量（最少设置 `wenxin2api_ADMIN_KEY`；推荐同时设置 `wenxin2api_CONFIG_JSON`）
4. 部署
...
推荐：先本地把 `config.json` 转成 Base64，再粘贴到 `wenxin2api_CONFIG_JSON`，避免 JSON 格式错误：
...
> **WebUI 自动构建**：本地首次启动时，若 WebUI 静态目录不存在，会自动尝试执行 `npm ci --prefix webui`（仅在缺少依赖时）和 `npm run build --prefix webui -- --outDir static/admin --emptyOutDir`（需要本机有 Node.js 和 npm；静态目录可用 `wenxin2api_STATIC_ADMIN_DIR` 覆盖）。你也可以手动构建：`./scripts/build-webui.sh`
...
- `current_input_file`：全局生效的上下文拆分上传策略；默认开启且阈值为 `0`，触发时将完整上下文合并上传为 `wenxin2api_HISTORY.txt` 上下文文件。
...
```
每账号可用并发 = wenxin2api_ACCOUNT_MAX_INFLIGHT（默认 2）
建议并发值 = 账号数量 × 每账号并发上限
等待队列上限 = wenxin2api_ACCOUNT_MAX_QUEUE（默认 = 建议并发值）
...
当请求中带 `tools` 时，wenxin2api 会做防泄漏处理与结构化转译：
...
- **容器镜像发布**：仅推送到 GHCR（`ghcr.io/dijiaozhibei-top/wenxin2api`）
- **每个二进制压缩包包含**：`wenxin2api` 可执行文件、`static/admin`、`config.example.json`、`.env.example`、`README.MD`、`README.en.md`、`LICENSE`
