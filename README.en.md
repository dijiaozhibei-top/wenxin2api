<p align="center">
  <img src="webui/public/wenxin2api-favicon.svg" width="128" height="128" alt="wenxin2api icon" />
</p>
...
flowchart LR
    ...
    subgraph wenxin2api["wenxin2api 4.x (Modular HTTP Surface + PromptCompat Core)"]
...
OpenAI `/v1/*` routes remain canonical, and wenxin2api also accepts root shortcuts such as `/models`, `/chat/completions`, `/responses`, `/embeddings`, `/files`, and `/files/{file_id}` for clients configured with the bare service URL.
...
Besides native IDs, wenxin2api also accepts common aliases as input (for example `gpt-4.1`, `gpt-5`, `gpt-5-codex`, `o3`, `claude-*`, `gemini-*`), but `/v1/models` returns normalized DeepSeek native model IDs. The complete alias behavior is documented in [API.en.md](API.en.md#model-alias-resolution) and `config.example.json`.
...
- Set `ANTHROPIC_BASE_URL` to the wenxin2api root URL (for example `http://127.0.0.1:5001`). Claude Code sends requests to `/v1/messages?beta=true`.
...
- If your environment has proxy variables, set `NO_PROXY=127.0.0.1,localhost,<your_host_ip>` for wenxin2api to avoid proxy interception of local traffic.
- If tool calls are rendered as plain text and not executed, first verify the model output uses the recommended halfwidth-pipe DSML block: `<|DSML|tool_calls><|DSML|invoke name="..."><|DSML|parameter name="...">...`. wenxin2api also accepts legacy canonical XML: `<tool_calls><invoke name="..."><parameter name="...">...`; legacy `<tools>` / `<tool_call>` / `<tool_name>` / `<param>`, `<function_call>`, `tool_use`, or standalone JSON `tool_calls` are not executed and stay plain text.
...
- Docker / Vercel: generate Base64 from `config.json` and inject as `wenxin2api_CONFIG_JSON`, or paste raw JSON directly
...
2. After deployment, open `/admin` and login with `wenxin2api_ADMIN_KEY` shown in Zeabur env/template instructions.
3. Import / edit config in Admin UI (it will be written and persisted to `/data/config.json`).

Fresh Zeabur volumes can start without `/data/config.json`; wenxin2api will boot with an empty file-backed config and create the file on the first Admin UI save.

For manual deployment without the template, create a Zeabur GitHub service, keep Root Directory as `/`, build with the repo-root `Dockerfile`, mount a persistent volume at `/data`, set `PORT=5001`, `wenxin2api_ADMIN_KEY=your-strong-secret`, and `wenxin2api_CONFIG_PATH=/data/config.json`, then expose HTTP port `5001`. See [docs/DEPLOY.en.md](docs/DEPLOY.en.md#manual-deployment-without-the-template) for the full guide.
...
3. Set environment variables (minimum: `wenxin2api_ADMIN_KEY`; recommended to also set `wenxin2api_CONFIG_JSON`)
4. Deploy
...
Recommended: convert `config.json` to Base64 locally, then paste into `wenxin2api_CONFIG_JSON` to avoid JSON formatting mistakes:
...
> **WebUI auto-build**: On first local startup, if the WebUI static directory is missing, wenxin2api auto-runs `npm ci --prefix webui` (only when dependencies are missing) and `npm run build --prefix webui -- --outDir static/admin --emptyOutDir` (requires Node.js; `wenxin2api_STATIC_ADMIN_DIR` can override the static directory). You can also build manually: `./scripts/build-webui.sh`
...
- `current_input_file`: the global context split/upload mode; it is enabled by default and uploads the full context as a `wenxin2api_HISTORY.txt` context file once the character threshold is reached.
...
For business endpoints (`/v1/*`, `/anthropic/*`, Gemini routes), wenxin2api supports two modes:

| Mode | Description |
| --- | --- |
| **Managed account** | Use a key from `config.keys` via `Authorization: Bearer ...` or `x-api-key`; wenxin2api auto-selects an account |
| **Direct token** | If the token is not in `config.keys`, wenxin2api treats it as a DeepSeek token directly |
...
```
Per-account inflight = wenxin2api_ACCOUNT_MAX_INFLIGHT (default 2)
Recommended concurrency = account_count × per_account_inflight
Queue limit = wenxin2api_ACCOUNT_MAX_QUEUE (default = recommended concurrency)
...
When `tools` is present in the request, wenxin2api performs anti-leak handling:
...
This is for debugging issues such as Responses reasoning streaming and tool-call handoff. When enabled, wenxin2api stores the latest N DeepSeek conversation payload pairs (request body + upstream response body), defaulting to 20 entries with auto-eviction; each response body is capped at 5 MB by default.
