...
- Docker / Vercel: generate Base64 from `config.json`, then set `wenxin2api_CONFIG_JSON`, or paste raw JSON directly

```bash
wenxin2api_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
```

For Vercel one-click bootstrap, you can set only `wenxin2api_ADMIN_KEY` first, then import config at `/admin` and sync env vars from the "Vercel Sync" page.
...
- Token is in `config.keys` → **Managed account mode**: wenxin2api auto-selects an account via rotation
...
OpenAI `/v1/*` paths are canonical. For clients configured with the bare wenxin2api service URL, the same OpenAI handlers are also exposed through root shortcuts: `/models`, `/models/{id}`, `/chat/completions`, `/responses`, `/responses/{response_id}`, `/embeddings`, `/files`, and `/files/{file_id}`.
...
For `chat` / `responses` / `embeddings`, wenxin2api follows a wide-input/strict-output policy:
...
> Path note: besides the canonical `/v1/chat/completions`, wenxin2api also accepts the root shortcut `/chat/completions`. On Vercel Runtime, `vercel.json` rewrites only the canonical `/v1/chat/completions` path to the Node streaming bridge; the root shortcut stays on the Go primary path. Use `/v1/chat/completions` on Vercel when real-time streaming is required.
...
When `tools` is present, wenxin2api performs anti-leak handling:
...
**Stream**: Once high-confidence toolcall features are matched, wenxin2api emits `delta.tool_calls` immediately (without waiting for full argument closure), then keeps sending argument deltas; confirmed tool-call fragments are not forwarded as `delta.content`.
...
If `tool_choice=required` and no valid tool call is produced, wenxin2api returns HTTP `422` (`error.code=tool_choice_violation`).
...
If `tool_choice=required` is violated in stream mode, wenxin2api emits `response.failed` then `[DONE]` (no `response.completed`).
...
Besides `/anthropic/v1/*`, wenxin2api also supports shortcut paths: `/v1/messages`, `/messages`, `/v1/messages/count_tokens`, `/messages/count_tokens`.
...
> `anthropic-version` is optional; wenxin2api auto-fills `2023-06-01` when absent.
...
```json
{
  ...
  "release_url": "https://github.com/dijiaozhibei-top/wenxin2api/releases/tag/v3.0.0",
