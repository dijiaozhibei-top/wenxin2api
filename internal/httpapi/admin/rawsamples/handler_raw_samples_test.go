...
func TestCaptureRawSampleWritesPersistentSample(t *testing.T) {
	t.Setenv("wenxin2api_RAW_STREAM_SAMPLE_ROOT", t.TempDir())
	...
	if got := rec.Header().Get("X-Ds2-Sample-Upstream"); got != filepath.Join(os.Getenv("wenxin2api_RAW_STREAM_SAMPLE_ROOT"), "my-sample-01", "upstream.stream.sse") {
	...
	sampleDir := filepath.Join(os.Getenv("wenxin2api_RAW_STREAM_SAMPLE_ROOT"), "my-sample-01")
...
func TestCaptureRawSampleCombinesContinuationCaptures(t *testing.T) {
	...
	sampleDir := filepath.Join(os.Getenv("wenxin2api_RAW_STREAM_SAMPLE_ROOT"), "my-sample-02")
...
func TestCaptureRawSampleReturnsErrorWhenNoNewCaptureRecorded(t *testing.T) {
	root := t.TempDir()
	t.Setenv("wenxin2api_RAW_STREAM_SAMPLE_ROOT", root)
	devcapture.Global().Clear()
	defer devcapture.Global().Clear()
