...
func TestSaveLocalVercelCredentialsStoresExplicitInput(t *testing.T) {
	t.Setenv("wenxin2api_CONFIG_JSON", `{"keys":["k1"]}`)
...
func TestSaveLocalVercelCredentialsPreservesPreconfiguredTokenAndUpdatesProject(t *testing.T) {
	t.Setenv("wenxin2api_CONFIG_JSON", `{"keys":["k1"],"vercel":{"token":"saved-token","project_id":"old-project","team_id":"old-team"}}`)
	store := config.LoadStore()
	h := &Handler{Store: store}
...
func TestExportSyncConfigStripsSavedVercelCredentials(t *testing.T) {
	t.Setenv("wenxin2api_CONFIG_JSON", `{"keys":["k1"],"vercel":{"token":"secret-token","project_id":"project","team_id":"team"}}`)
	store := config.LoadStore()
	h := &Handler{Store: store}
