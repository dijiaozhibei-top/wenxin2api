...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "# wenxin2api_HISTORY.txt") {
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if upload.Filename != "wenxin2api_HISTORY.txt" {
	...
	if !strings.Contains(out.FinalPrompt, "Continue from the latest state in the attached wenxin2api_HISTORY.txt context.") {
	...
	if !strings.Contains(out.PromptTokenText, "# wenxin2api_HISTORY.txt") || !strings.Contains(out.PromptTokenText, "=== 1. USER ===") {
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "# wenxin2api_HISTORY.txt") || !strings.Contains(out.PromptTokenText, "=== 1. SYSTEM ===") {
		t.Fatalf("expected prompt token text to include numbered history transcript, got %q", out.PromptTokenText)
	}
	if !strings.Contains(out.PromptTokenText, "Continue from the latest state in the attached wenxin2api_HISTORY.txt context.") {
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	upload := ds.uploadCalls[0]
	if upload.Filename != "wenxin2api_HISTORY.txt" {
		t.Fatalf("expected wenxin2api_HISTORY.txt upload, got %q", upload.Filename)
	}
	if upload.ModelType != "vision" {
		t.Fatalf("expected vision model type for vision request, got %q", upload.ModelType)
	}
	uploadedText := string(upload.Data)
	for _, want := range []string{"# wenxin2api_HISTORY.txt", "=== 1. SYSTEM ===", "=== 2. USER ===", "=== 3. ASSISTANT ===", "=== 4. TOOL ===", "=== 5. USER ===", "system instructions", "first user turn", "hidden reasoning", "tool result", "latest user turn", promptcompat.ThinkingInjectionMarker} {
		if !strings.Contains(uploadedText, want) {
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "wenxin2api_HISTORY.txt" {
		t.Fatalf("expected first upload to be wenxin2api_HISTORY.txt, got %q", ds.uploadCalls[0].Filename)
	}
	if ds.uploadCalls[1].Filename != "wenxin2api_TOOLS.txt" {
		t.Fatalf("expected second upload to be wenxin2api_TOOLS.txt, got %q", ds.uploadCalls[1].Filename)
	}
	...
	for _, want := range []string{"# wenxin2api_TOOLS.txt", "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
	...
	if !strings.Contains(out.FinalPrompt, "Continue from the latest state in the attached wenxin2api_HISTORY.txt context.") || !strings.Contains(out.FinalPrompt, "wenxin2api_TOOLS.txt") {
	...
	if !strings.Contains(out.PromptTokenText, "# wenxin2api_HISTORY.txt") || !strings.Contains(out.PromptTokenText, "# wenxin2api_TOOLS.txt") || !strings.Contains(out.PromptTokenText, "Description: search docs") {
...
func TestApplyCurrentInputFileCarriesHistoryText(t *testing.T) {
	...
	if !strings.Contains(out.HistoryText, "# wenxin2api_HISTORY.txt") || !strings.Contains(out.HistoryText, "=== 1. SYSTEM ===") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "# wenxin2api_HISTORY.txt") || !strings.Contains(historyText, "=== 1. SYSTEM ===") {
	...
	if !strings.Contains(promptText, "Continue from the latest state in the attached wenxin2api_HISTORY.txt context.") {
...
func TestResponsesCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "wenxin2api_HISTORY.txt" || ds.uploadCalls[1].Filename != "wenxin2api_TOOLS.txt" {
	...
	toolsText := string(ds.uploadCalls[1].Data)
	if !strings.Contains(toolsText, "# wenxin2api_TOOLS.txt") || !strings.Contains(toolsText, "Tool: search") || !strings.Contains(toolsText, "Description: search docs") {
		t.Fatalf("expected tools transcript to include schema, got %q", toolsText)
	}
	promptText, _ := ds.completionReq["prompt"].(string)
	if !strings.Contains(promptText, "wenxin2api_TOOLS.txt") || !strings.Contains(promptText, "TOOL CALL FORMAT") {
...
func TestCurrentInputFileWorksAcrossAutoDeleteModes(t *testing.T) {
	for _, mode := range []string{"none", "single", "all"} {
		t.Run(mode, func(t *testing.T) {
			...
			promptText, _ := ds.completionReq["prompt"].(string)
			if !strings.Contains(promptText, "Continue from the latest state in the attached wenxin2api_HISTORY.txt context.") || strings.Contains(promptText, "first user turn") || strings.Contains(promptText, "latest user turn") {
