...
func TestVercelInternalSecret(t *testing.T) {
	t.Run("prefer explicit secret", func(t *testing.T) {
		t.Setenv("wenxin2api_VERCEL_INTERNAL_SECRET", "stream-secret")
		t.Setenv("wenxin2api_ADMIN_KEY", "admin-fallback")
	...
	t.Run("fallback to admin key", func(t *testing.T) {
		t.Setenv("wenxin2api_VERCEL_INTERNAL_SECRET", "")
		t.Setenv("wenxin2api_ADMIN_KEY", "admin-fallback")
...
func TestStreamLeaseTTL(t *testing.T) {
	t.Setenv("wenxin2api_VERCEL_STREAM_LEASE_TTL_SECONDS", "120")
	if got := streamLeaseTTL(); got != 120*time.Second {
		t.Fatalf("expected ttl=120s, got %v", got)
	}
	t.Setenv("wenxin2api_VERCEL_STREAM_LEASE_TTL_SECONDS", "invalid")
...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             "Continue from the latest state in the attached wenxin2api_HISTORY.txt context. Available tool descriptions and parameter schemas are attached in wenxin2api_TOOLS.txt; use only those tools and follow the tool-call format rules in this prompt.",
		PromptTokenText:         "# wenxin2api_HISTORY.txt\n\n=== 1. USER ===\nhello\n\n# wenxin2api_TOOLS.txt\nAvailable tool descriptions and parameter schemas for this request.\n\nYou have access to these tools:\n\nTool: search\nDescription: search docs\nParameters: {\"type\":\"object\"}\n",
		HistoryText:             "# wenxin2api_HISTORY.txt\n\n=== 1. USER ===\nhello\n",
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, "wenxin2api_TOOLS.txt") {
