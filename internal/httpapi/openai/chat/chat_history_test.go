...
func TestChatCompletionsCurrentInputFilePersistsNeutralPrompt(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "wenxin2api_HISTORY.txt" {
		t.Fatalf("expected wenxin2api_HISTORY.txt upload, got %q", ds.uploadCalls[0].Filename)
	}
	...
	if !strings.Contains(full.Messages[0].Content, "Continue from the latest state in the attached wenxin2api_HISTORY.txt context.") {
