...
func effectiveAdminKey(store AdminConfigReader) string {
	...
	warnOnce.Do(func() {
		slog.Warn("⚠️  wenxin2api_ADMIN_KEY is not set! Using insecure default \"admin\". Set a strong key in production!")
	})
	return "admin"
}

func jwtSecret(store AdminConfigReader) string {
	if v := strings.TrimSpace(os.Getenv("wenxin2api_JWT_SECRET")); v != "" {
		return v
	}
	if store != nil {
		if hash := strings.TrimSpace(store.AdminPasswordHash()); hash != "" {
...
func jwtExpireHours(store AdminConfigReader) int {
	...
	if v := strings.TrimSpace(os.Getenv("wenxin2api_JWT_EXPIRE_HOURS")); v != "" {
...
func UsingDefaultAdminKey(store AdminConfigReader) bool {
	...
	return strings.TrimSpace(os.Getenv("wenxin2api_ADMIN_KEY")) == ""
}
