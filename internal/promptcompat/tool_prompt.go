...
const CurrentToolsContextFilename = "wenxin2api_TOOLS.txt"

const toolsTranscriptTitle = "# wenxin2api_TOOLS.txt"
...
func injectToolPromptWithDescriptions(messages []map[string]any, tools []any, policy ToolChoicePolicy, includeDescriptions bool) ([]map[string]any, []string) {
	...
	if includeDescriptions && parts.Descriptions != "" {
		...
		toolPrompt = "Available tool descriptions and parameter schemas are attached in wenxin2api_TOOLS.txt. Treat wenxin2api_TOOLS.txt as the authoritative list of callable tools and schemas; use only tools and parameters listed there.\n\n" + toolPrompt
	}
