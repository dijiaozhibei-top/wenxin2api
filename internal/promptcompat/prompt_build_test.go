...
func TestBuildOpenAIPromptWithToolInstructionsOnlyOmitsSchemas(t *testing.T) {
	...
	if !strings.Contains(finalPrompt, "Treat wenxin2api_TOOLS.txt as the authoritative list of callable tools and schemas") {
...
func TestBuildOpenAIToolsContextTranscriptContainsOnlyDescriptions(t *testing.T) {
	...
	for _, want := range []string{"# wenxin2api_TOOLS.txt", "You have access to these tools", "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
