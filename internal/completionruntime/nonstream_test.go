...
func TestStartCompletionAppliesCurrentInputFileGlobally(t *testing.T) {
	...
	if got := ds.uploads[0].Filename; got != "wenxin2api_HISTORY.txt" {
		t.Fatalf("upload filename=%q want wenxin2api_HISTORY.txt", got)
	}
	...
	if !start.Request.CurrentInputFileApplied || !strings.Contains(start.Request.PromptTokenText, "# wenxin2api_HISTORY.txt") {
