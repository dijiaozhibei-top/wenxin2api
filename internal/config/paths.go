...
func ConfigPath() string {
	if strings.TrimSpace(os.Getenv("wenxin2api_CONFIG_PATH")) == "" && BaseDir() == "/app" {
		return containerDefaultConfigPath()
	}
	return ResolvePath("wenxin2api_CONFIG_PATH", "config.json")
}
...
func shouldTryLegacyContainerConfigPath() bool {
	return strings.TrimSpace(os.Getenv("wenxin2api_CONFIG_PATH")) == "" && BaseDir() == "/app"
}

func RawStreamSampleRoot() string {
	return ResolvePath("wenxin2api_RAW_STREAM_SAMPLE_ROOT", "tests/raw_stream_samples")
}
...
func ChatHistoryPath() string {
	...
	if IsVercel() && strings.TrimSpace(os.Getenv("wenxin2api_CHAT_HISTORY_PATH")) == "" {
		return "/tmp/chat_history.json"
	}
	return ResolvePath("wenxin2api_CHAT_HISTORY_PATH", "data/chat_history.json")
}

func StaticAdminDir() string {
	return ResolvePath("wenxin2api_STATIC_ADMIN_DIR", "static/admin")
}
