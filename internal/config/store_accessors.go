...
func (s *Store) AdminJWTExpireHours() int {
	...
	if raw := strings.TrimSpace(os.Getenv("wenxin2api_JWT_EXPIRE_HOURS")); raw != "" {
...
func (s *Store) RuntimeAccountMaxInflight() int {
	...
	if raw := strings.TrimSpace(os.Getenv("wenxin2api_ACCOUNT_MAX_INFLIGHT")); raw != "" {
		if n, err := strconv.Atoi(raw); err == nil && n > 0 {
			return n
		}
	}
...
func (s *Store) RuntimeAccountMaxQueue(defaultSize int) int {
	...
	if raw := strings.TrimSpace(os.Getenv("wenxin2api_ACCOUNT_MAX_QUEUE")); raw != "" {
		if n, err := strconv.Atoi(raw); err == nil && n >= 0 {
			return n
		}
	}
...
func (s *Store) RuntimeGlobalMaxInflight(defaultSize int) int {
	...
	if raw := strings.TrimSpace(os.Getenv("wenxin2api_GLOBAL_MAX_INFLIGHT")); raw != "" {
		if n, err := strconv.Atoi(raw); err == nil && n > 0 {
			return n
		}
	}
