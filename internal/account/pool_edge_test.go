...
func TestPoolEmptyNoAccounts(t *testing.T) {
	t.Setenv("wenxin2api_ACCOUNT_MAX_INFLIGHT", "2")
	t.Setenv("wenxin2api_ACCOUNT_MAX_QUEUE", "")
...
func TestPoolMaxQueueSizeOverride(t *testing.T) {
	t.Setenv("wenxin2api_ACCOUNT_MAX_INFLIGHT", "1")
	t.Setenv("wenxin2api_ACCOUNT_MAX_QUEUE", "5")
	t.Setenv("wenxin2api_CONFIG_JSON", `{"keys":["k1"],"accounts":[{"email":"acc1@example.com","token":"t1"}]}`)
	pool := NewPool(config.LoadStore())
	status := pool.Status()
