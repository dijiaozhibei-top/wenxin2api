...
- **环境变量方式**：`wenxin2api_CONFIG_JSON`（推荐 Vercel 使用，支持 JSON 字符串或 Base64 编码，也可以直接写原始 JSON）
...
- Docker / Vercel：从 `config.json` 生成 `wenxin2api_CONFIG_JSON`（Base64）注入环境变量
...
- **容器镜像发布**：仅发布到 GHCR（`ghcr.io/dijiaozhibei-top/wenxin2api`）
...
- `wenxin2api` 可执行文件（Windows 为 `wenxin2api.exe`）
...
默认 `docker-compose.yml` 直接使用 `ghcr.io/dijiaozhibei-top/wenxin2api:latest`，并把宿主机 `6011` 映射到容器内的 `5001`。如果你希望直接对外暴露 `5001`，请设置 `wenxin2api_HOST_PORT=5001`（或者手动调整 `ports` 配置）。
Compose 模板还会默认设置 `wenxin2api_CONFIG_PATH=/data/config.json` 并挂载 `./config.json:/data/config.json`，优先避免 `/app` 只读带来的配置持久化问题。
镜像内会预创建 `/data` 并授权给非 root 的 `wenxin2api` 用户；如果你使用 bind mount 单文件，请确保宿主机 `config.json` 至少可被容器用户读取/写入，例如 `chmod 644 config.json`，否则 Linux UID/GID 不一致时仍可能出现 `open /data/config.json: permission denied`。
兼容说明：若未设置 `wenxin2api_CONFIG_PATH` 且运行目录是 `/app`，新版本会优先使用 `/data/config.json`；当该文件不存在但检测到历史 `/app/config.json` 时，会自动回退读取旧路径，避免升级后“配置丢失”。
...
容器内启动命令：`/usr/local/bin/wenxin2api`，默认暴露端口 `5001`。
...
- **配置持久化**：模板挂载卷 `/data`，并设置 `wenxin2api_CONFIG_PATH=/data/config.json`；首次空卷启动时会先使用空的文件模式配置，在管理台导入配置后，会写入并持久化到该路径。
- **`open /app/config.json: permission denied`**：说明当前实例在尝试把运行时 token 持久化到只读路径（常见于镜像内 `/app`）。  
  处理建议：
  1. 显式设置可写路径：`wenxin2api_CONFIG_PATH=/data/config.json`（并挂载持久卷到 `/data`）；  
  2. 若你使用 `wenxin2api_CONFIG_JSON` 启动且不需要运行时落盘，可保持环境变量模式（`wenxin2api_ENV_WRITEBACK` 关闭）；  
...
- **首次登录**：部署完成后访问 `/admin`，使用 Zeabur 环境变量/模板指引中的 `wenxin2api_ADMIN_KEY` 登录（建议首次登录后自行更换为强密码）。
...
| `wenxin2api_ADMIN_KEY` | 强随机字符串 | 管理台登录密钥，必填。 |
| `wenxin2api_CONFIG_PATH` | `/data/config.json` | 配置持久化路径，建议必填。 |
| `LOG_LEVEL` | `INFO` | 可选，日志级别。 |
| `wenxin2api_CONFIG_JSON` | 原始 JSON 或 Base64 JSON | 可选，用于用环境变量初始化配置。 |
| `wenxin2api_ENV_WRITEBACK` | `1` | 可选；当设置了 `wenxin2api_CONFIG_JSON` 且希望首次启动后写入 `/data/config.json` 时再启用。 |

7. 暴露 HTTP 端口 `5001`，健康检查路径可填 `/healthz`。
8. 部署完成后访问 `/admin`，用 `wenxin2api_ADMIN_KEY` 登录，然后在管理台导入或编辑配置。首次空卷可以没有 `/data/config.json`，服务会先启动，第一次保存时自动创建该文件。

常见问题：

- **启动日志出现 `open /data/config.json: no such file or directory`**：请确认已经部署包含“首次空卷启动”修复的版本，并重新部署最新代码。
- **出现 `open /app/config.json: permission denied`**：说明配置路径仍指向镜像内只读目录；设置持久卷 `/data`，并确认 `wenxin2api_CONFIG_PATH=/data/config.json`。
- **管理台保存后重启配置丢失**：检查 `/data` 持久卷是否已挂载到当前服务；如果使用了 `wenxin2api_CONFIG_JSON`，但想让管理台保存落盘，请启用 `wenxin2api_ENV_WRITEBACK=1`。
...
| 变量 | 说明 |
| --- | --- |
| `wenxin2api_ADMIN_KEY` | 管理密钥（必填） |
| `wenxin2api_CONFIG_JSON` | 配置内容，JSON 字符串或 Base64 编码（可选，建议） |

4. **部署**

### 3.1.1 推荐填写方式（避免 `wenxin2api_CONFIG_JSON` 填错）

如果你想先完成一键部署，也可以先不填 `wenxin2api_CONFIG_JSON`，部署后进入 `/admin` 导入配置，再在「Vercel 同步」里写回环境变量。
...
```text
wenxin2api_ADMIN_KEY=请替换为强密码
wenxin2api_CONFIG_JSON=上一步生成的一整行 Base64
```
...
| 变量 | 说明 | 默认值 |
| --- | --- | --- |
| `wenxin2api_ACCOUNT_MAX_INFLIGHT` | 每账号并发上限 | `2` |
| `wenxin2api_ACCOUNT_MAX_QUEUE` | 等待队列上限 | `recommended_concurrency` |
| `wenxin2api_GLOBAL_MAX_INFLIGHT` | 全局并发上限 | `recommended_concurrency` |
| `wenxin2api_ENV_WRITEBACK` | 检测到 `wenxin2api_CONFIG_JSON` 时自动写入 `wenxin2api_CONFIG_PATH`，并在成功后转为文件模式（`1/true/yes/on`） | 关闭 |
| `wenxin2api_VERCEL_INTERNAL_SECRET` | 混合流式内部鉴权 | 回退用 `wenxin2api_ADMIN_KEY` |
| `wenxin2api_VERCEL_STREAM_LEASE_TTL_SECONDS` | 流式 lease TTL | `900` |
| `wenxin2api_RAW_STREAM_SAMPLE_ROOT` | raw stream 样本保存/读取根目录 | `tests/raw_stream_samples` |
| `wenxin2api_STATIC_ADMIN_DIR` | WebUI 静态资源目录 | `static/admin` |
| `wenxin2api_AUTO_BUILD_WEBUI` | 本地启动时是否自动构建缺失的 WebUI（`1/true/yes/on` 或 `0/false/no/off`） | 非 Vercel 默认开启 |
| `VERCEL_TOKEN` | Vercel 同步 token | — |
| `VERCEL_PROJECT_ID` | Vercel 项目 ID | — |
| `VERCEL_TEAM_ID` | Vercel 团队 ID | — |
| `wenxin2api_CHAT_HISTORY_PATH` | Chat history 存储路径（Vercel 上必须设为 `/tmp/chat_history.json`，否则因文件系统只读而不可用） | `data/chat_history.json` |
| `wenxin2api_VERCEL_PROTECTION_BYPASS` | 部署保护绕过密钥（内部 Node→Go 调用） | — |
...
**解决**：当前仓库已通过公开桥接包 `app` 解决：`api/index.go` → `wenxin2api/app` → `internal/server`。
...
- **方案 C**：设置 `VERCEL_AUTOMATION_BYPASS_SECRET`（或 `wenxin2api_VERCEL_PROTECTION_BYPASS`），仅影响内部 Node→Go 调用
...
本地首次启动时，若 WebUI 静态目录不存在，服务会自动尝试构建 WebUI（需要 Node.js/npm；缺依赖时会先执行 `npm ci --prefix webui`，再执行 `npm run build --prefix webui -- --outDir <静态目录> --emptyOutDir`）。默认静态目录为 `static/admin/`，可用 `wenxin2api_STATIC_ADMIN_DIR` 覆盖。
