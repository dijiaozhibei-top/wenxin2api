...
- **容器镜像发布**：仅发布到 GHCR（`ghcr.io/cjackhwang/wenxin2api`）
...
默认 `docker-compose.yml` 直接使用 `ghcr.io/cjackhwang/wenxin2api:latest`，并把宿主机 `6011` 映射到容器内的 `5001`。如果你希望直接对外暴露 `5001`，请设置 `wenxin2api_HOST_PORT=5001`（或者手动调整 `ports` 配置）。
