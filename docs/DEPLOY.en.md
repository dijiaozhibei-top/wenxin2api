...
- **Environment variable**: `wenxin2api_CONFIG_JSON` (recommended for Vercel; supports raw JSON or Base64)
...
- Docker / Vercel: generate `wenxin2api_CONFIG_JSON` (Base64) from `config.json` and inject it
...
- `wenxin2api` executable (`wenxin2api.exe` on Windows)
...
# 2. Extract
tar -xzf wenxin2api_<tag>_linux_amd64.tar.gz
cd wenxin2api_<tag>_linux_amd64
...
#   wenxin2api_ADMIN_KEY=your-admin-key
# Optionally set the host port:
#   wenxin2api_HOST_PORT=6011
...
The default `docker-compose.yml` directly uses `ghcr.io/dijiaozhibei-top/wenxin2api:latest` and maps host port `6011` to container port `5001`. If you want `5001` exposed directly, set `wenxin2api_HOST_PORT=5001` (or adjust the `ports` mapping).
The compose template also defaults to `wenxin2api_CONFIG_PATH=/data/config.json` with `./config.json:/data/config.json` mounted, so deployments avoid read-only `/app` persistence issues by default.
The image pre-creates `/data` and grants it to the non-root `wenxin2api` user. If you bind-mount a single host file, make sure `config.json` is readable/writable by the container user, for example with `chmod 644 config.json`; otherwise Linux UID/GID mismatches can still cause `open /data/config.json: permission denied`.
Compatibility note: when `wenxin2api_CONFIG_PATH` is unset and runtime base dir is `/app`, newer versions prefer `/data/config.json`; if that file is missing but legacy `/app/config.json` exists, wenxin2api automatically falls back to the legacy path to avoid post-upgrade config loss.
...
Container entry command: `/usr/local/bin/wenxin2api`, default exposed port: `5001`.
...
- **Port**: wenxin2api listens on `5001` by default; the template sets `PORT=5001`.
- **Persistent config**: the template mounts `/data` and sets `wenxin2api_CONFIG_PATH=/data/config.json`. On a fresh volume, wenxin2api starts with an empty file-backed config; after importing config in Admin UI, it will be written and persisted to this path.
- **`open /app/config.json: permission denied`**: this means the instance is trying to persist runtime tokens to a read-only path (commonly `/app` inside the image).  
  ...
  1. Set a writable path explicitly: `wenxin2api_CONFIG_PATH=/data/config.json` (and mount a persistent volume at `/data`);
  2. If you bootstrap with `wenxin2api_CONFIG_JSON` and do not need runtime writeback, keep env-backed mode (`wenxin2api_ENV_WRITEBACK` disabled);
...
- **First login**: after deployment, open `/admin` and login with `wenxin2api_ADMIN_KEY` shown in Zeabur env/template instructions (recommended: rotate to a strong secret after first login).
...
| `wenxin2api_ADMIN_KEY` | Strong random string | Required admin login key. |
| `wenxin2api_CONFIG_PATH` | `/data/config.json` | Recommended persistent config path. |
| `LOG_LEVEL` | `INFO` | Optional log level. |
| `wenxin2api_CONFIG_JSON` | Raw JSON or Base64 JSON | Optional config bootstrap from env. |
| `wenxin2api_ENV_WRITEBACK` | `1` | Optional; enable only when using `wenxin2api_CONFIG_JSON` and you want the initial config written to `/data/config.json`. |

7. Expose HTTP port `5001`. The health check path can be `/healthz`.
8. After deployment, open `/admin`, login with `wenxin2api_ADMIN_KEY`, then import or edit config in Admin UI. A fresh volume does not need `/data/config.json` up front; the service boots first and creates the file on the first save.
...
- **`open /app/config.json: permission denied`**: the config path still points at the read-only image directory; mount `/data` and set `wenxin2api_CONFIG_PATH=/data/config.json`.
- **Config disappears after restart**: check that the `/data` persistent volume is mounted on this service. If you use `wenxin2api_CONFIG_JSON` but want Admin UI saves persisted, enable `wenxin2api_ENV_WRITEBACK=1`.
...
| --- | --- |
| `wenxin2api_ADMIN_KEY` | Admin key (required) |
| `wenxin2api_CONFIG_JSON` | Config content, raw JSON or Base64 (optional, recommended) |

4. **Deploy**

### 3.1.1 Recommended Input (avoid `wenxin2api_CONFIG_JSON` mistakes)

If you prefer faster one-click bootstrap, you can leave `wenxin2api_CONFIG_JSON` empty first, then open `/admin` after deployment, import config, and sync it back to Vercel env vars from the "Vercel Sync" page.
...
```bash
# Run in repo root
wenxin2api_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
echo "$wenxin2api_CONFIG_JSON"
```

If you choose to preconfigure before first deploy, set these vars in Vercel Project Settings -> Environment Variables:

```text
wenxin2api_ADMIN_KEY=replace-with-a-strong-secret
wenxin2api_CONFIG_JSON=<the single-line Base64 output above>
```
...
| --- | --- | --- |
| `wenxin2api_ACCOUNT_MAX_INFLIGHT` | Per-account inflight limit | `2` |
| `wenxin2api_ACCOUNT_MAX_QUEUE` | Waiting queue limit | `recommended_concurrency` |
| `wenxin2api_GLOBAL_MAX_INFLIGHT` | Global inflight limit | `recommended_concurrency` |
| `wenxin2api_ENV_WRITEBACK` | When `wenxin2api_CONFIG_JSON` is present, auto-write to `wenxin2api_CONFIG_PATH` and switch to file-backed mode after success (`1/true/yes/on`) | Disabled |
| `wenxin2api_VERCEL_INTERNAL_SECRET` | Hybrid streaming internal auth | Falls back to `wenxin2api_ADMIN_KEY` |
| `wenxin2api_VERCEL_STREAM_LEASE_TTL_SECONDS` | Stream lease TTL | `900` |
| `wenxin2api_RAW_STREAM_SAMPLE_ROOT` | Raw stream sample root for saving/reading samples | `tests/raw_stream_samples` |
| `wenxin2api_STATIC_ADMIN_DIR` | WebUI static asset directory | `static/admin` |
| `wenxin2api_AUTO_BUILD_WEBUI` | Whether local startup auto-builds missing WebUI assets (`1/true/yes/on` or `0/false/no/off`) | Enabled outside Vercel |
| `VERCEL_TOKEN` | Vercel sync token | — |
| `VERCEL_PROJECT_ID` | Vercel project ID | — |
| `VERCEL_TEAM_ID` | Vercel team ID | — |
| `wenxin2api_CHAT_HISTORY_PATH` | Chat history storage path (must be set to `/tmp/chat_history.json` on Vercel, otherwise unavailable due to read-only filesystem) | `data/chat_history.json` |
| `wenxin2api_VERCEL_PROTECTION_BYPASS` | Deployment protection bypass for internal Node→Go calls | — |
...
```text
use of internal package wenxin2api/internal/server not allowed
```

**Cause**: Vercel Go entrypoint directly imports `internal/...`.

**Fix**: This repo uses a public bridge package: `api/index.go` → `wenxin2api/app` → `internal/server`.
...
- **Option C**: Set `VERCEL_AUTOMATION_BYPASS_SECRET` (or `wenxin2api_VERCEL_PROTECTION_BYPASS`) for internal Node→Go calls
...
```text
wenxin2api_CHAT_HISTORY_PATH=/tmp/chat_history.json
```
...
On first local startup, if the WebUI static directory is missing, wenxin2api automatically attempts to build it (requires Node.js/npm; when dependencies are missing it runs `npm ci --prefix webui`, then `npm run build --prefix webui -- --outDir <static-dir> --emptyOutDir`). The default static directory is `static/admin/`, and `wenxin2api_STATIC_ADMIN_DIR` can override it.
...
sudo mkdir -p /opt/wenxin2api
sudo cp wenxin2api config.json /opt/wenxin2api/
sudo cp -r static/admin /opt/wenxin2api/static/admin
```

### 6.2 Create systemd Service File

```ini
# /etc/systemd/system/wenxin2api.service

[Unit]
Description=wenxin2api (Go)
After=network.target

[Service]
Type=simple
WorkingDirectory=/opt/wenxin2api
Environment=PORT=5001
Environment=wenxin2api_CONFIG_PATH=/opt/wenxin2api/config.json
Environment=wenxin2api_ADMIN_KEY=your-admin-key-here
ExecStart=/opt/wenxin2api/wenxin2api
Restart=always
RestartSec=5
...
# Enable on boot
sudo systemctl enable wenxin2api

# Start
sudo systemctl start wenxin2api

# Check status
sudo systemctl status wenxin2api

# View logs
sudo journalctl -u wenxin2api -f

# Restart
sudo systemctl restart wenxin2api

# Stop
sudo systemctl stop wenxin2api
```
