...
- Docker / Vercel：从 `config.json` 生成 Base64，填入 `wenxin2api_CONFIG_JSON`，也可以直接填原始 JSON

```bash
wenxin2api_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
```

Vercel 一键部署可先只填 `wenxin2api_ADMIN_KEY`，部署后在 `/admin` 导入配置，再通过 “Vercel 同步” 写回环境变量。
...
OpenAI `/v1/*` 仍是规范路径。对于只配置 wenxin2api 根地址的客户端，同一套 OpenAI handler 也通过根路径快捷路由暴露：`/models`、`/models/{id}`、`/chat/completions`、`/responses`、`/responses/{response_id}`、`/embeddings`、`/files`、`/files/{file_id}`。
...
当请求中含 `tools` 时，wenxin2api 做防泄漏处理：
